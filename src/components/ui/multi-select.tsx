...
interface MultiSelectProps
  ...
  /**
   * Controlled selected values. When provided, the component becomes fully controlled.
   * The component will display these values instead of managing its own state.
   * Use this with onValueChange for full control over the component state.
   * Optional, if not provided the component manages its own state.
   */
  value?: string[];
}
  ...
  (
    {
      ...
      closeOnSelect = false,
      value: controlledValue, // added controlled value prop
      ...props
    },
    ref
  ) => {
    ...
    const displayedValues = controlledValue ?? selectedValues;
    ...
    React.useEffect(() => {
      const selectedCount = displayedValues.length; // use displayedValues
      ...
      if (selectedCount !== prevSelectedCount.current) {
        ...
        if (diff > 0) {
          const addedItems = displayedValues.slice(-diff); // use displayedValues
    ...
    }, [displayedValues, isPopoverOpen, searchValue, announce, getAllOptions]); // use displayedValues
      ...
      <>
        ...
        >
          ...
          <div id={selectedCountId} className="sr-only" aria-live="polite">
            {displayedValues.length === 0 // use displayedValues
              ? "No options selected"
              : `${displayedValues.length} option${
                  displayedValues.length === 1 ? "" : "s"
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              aria-label={`Multi-select: ${displayedValues.length} of ${
                // use displayedValues
            ...
            >
              {displayedValues.length > 0 ? ( // use displayedValues
                <div className="flex justify-between items-center w-full">
                  ...
                  >
                    {displayedValues // use displayedValues
                      ...
                      .filter(Boolean)}
                    {displayedValues.length > responsiveSettings.maxCount && ( // use displayedValues
                      <Badge
                        className={cn(
                  ...
                  <div className="flex items-center justify-between">
                    <div
                      ...
                      aria-label={`Clear all ${displayedValues.length} selected options`} // use displayedValues
          ...
          >
            <Command>
              ...
              <CommandList
                className={cn(
                  "max-h-[30vh] overflow-y-auto multiselect-scrollbar",
                  screenSize === "mobile" && "max-h-[40vh]",
                ...
                {isGroupedOptions(filteredOptions) ? (
                  filteredOptions.map((group) => (
                    <CommandGroup key={group.heading} heading={group.heading}>
                      {group.options.map((option) => {
                        const isSelected = displayedValues.includes(
                          option.value
                        ); // use displayedValues
                        return (
                          <CommandItem
                  ...
                  <CommandGroup>
                    {filteredOptions.map((option) => {
                      const isSelected = displayedValues.includes(option.value); // use displayedValues
                      return (
                        <CommandItem
                          key={option.value}
                          onSelect={() => toggleOption(option.value)}
